...
#if defined (__IAR_SYSTEMS_ICC__) && (defined __CORTEX_M) && ((__CORTEX_M == 4U) || (__CORTEX_M == 7U) || (__CORTEX_M == 33U))
#define D_BARRIER    __asm("DSB"); /* __DSB() could not be used */
#else
#define D_BARRIER
#endif
#define ENABLE_GLOBAL_IRQ(reg) D_BARRIER; EnableGlobalIRQ(reg)
#define KB(x) ((x) << 10)
...
mem_status_t MEM_RegisterExtendedArea(memAreaCfg_t *area_desc, uint8_t *p_area_id, uint16_t flags)
{
    ...
    do {
        ...
        else
        {
            ...
            if ((flags & AREA_FLAGS_POOL_NOT_SHARED) != 0u)
            {
                /* The caller must pass a valid pointer to retrieve the area id in order to be able to allocate
                *  from it.
                **/
                assert(p_area_id != NULL);
                st = kStatus_MemInitError;
                break;
            }
...
uint32_t MEM_GetHeapUpperLimitByArea(uint8_t id)
{
    memAreaPrivDesc_t* p_area = MEM_GetAreaByAreaId(id);
    /* There is always a free block at the end of the heap area
       and this free block is the tail of the list */
    uint32_t up_limit = 0U;
    if (p_area != NULL)
    {
        up_limit = ((uint32_t)p_area->ctx.FreeBlockHdrList.tail + BLOCK_HDR_SIZE);
    }
    return up_limit;
}
...
{
    return MEM_GetHeapUpperLimitByArea(0U);
}
...
static uint32_t MEM_GetFreeHeapSpaceInArea(memAreaPrivDesc_t* p_area)
{
    ...
    free_sz += (uint32_t)p_area->end_address.raw_address - (uint32_t)p_area->ctx.FreeBlockHdrList.tail - BLOCK_HDR_SIZE + 1;
    return free_sz;
...
{
    return MEM_GetFreeHeapSizeByAreaId(0);
}
